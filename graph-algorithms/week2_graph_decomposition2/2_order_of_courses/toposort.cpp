...
class GraphExplorer {
  public:
    ...
    void ExploreSCC(Node& node, uint64_t scc_label, vector<uint64_t>& LinearOrder) {

      node.visited = true;
      node.scc_label = scc_label;
      LinearOrder.push_back(node.index + 1);

      // std::cout << "Exploring " << node.index + 1 << " | SCC: " << scc_label << "\n";

      for (const auto& adj_node_p: node.adj){
        if(false == (*adj_node_p).visited){
          ExploreSCC(*adj_node_p, scc_label, LinearOrder);
        }
      }
    ...
    vector<uint64_t> GetLinearOrder(){
      vector<uint64_t> LinearOrder;
      ...
      for(auto rnode_it = order.begin(); rnode_it != order.end(); ++rnode_it){
        ...
        if (node.scc_label == 0){
          ExploreSCC(node, scc_label, LinearOrder);
          ++scc_label;
        }
      }
      std::reverse(LinearOrder.begin(), LinearOrder.end());
      return LinearOrder;
    }
...
int main() {
  ...
  auto order = graph.GetLinearOrder();
  for (auto& o: order)
    std::cout << o << " ";
  return 0;
}
