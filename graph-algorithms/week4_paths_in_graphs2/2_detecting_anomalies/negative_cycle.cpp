...
class OptimalExchange{
  public:
    ...
    bool FindExchange(){
      ...
      InitializeUnexploredSet();

      bool negativeCycleFound = false;
      while (!m_UnexploredIndexes.empty()){
        uint64_t unexplored_index = *m_UnexploredIndexes.begin();

        InitializeGraphNodes();

        negativeCycleFound = SearchForNegativeCycle(unexplored_index);
        if (negativeCycleFound)
          break;
      }

      return negativeCycleFound; // If there has been a change in the |V|th cycle, there is a negative cycle
      // |V| == m_Graph.size() == number of nodes
    }

  private:

    Graph m_Graph;
    set<uint64_t> m_UnexploredIndexes;
    ...
    void InitializeUnexploredSet(){
      m_UnexploredIndexes.clear();
      for (Node& node: m_Graph){
        m_UnexploredIndexes.insert(node.m_Index);
      }
    }

    bool SearchForNegativeCycle(uint64_t start_node_index){
      m_Graph[start_node_index].m_Distance = 0;
      m_Graph[start_node_index].m_Reachable = true;
      m_UnexploredIndexes.erase(start_node_index);
      ...
      for (uint64_t cycle = 0; cycle <= m_Graph.size(); ++cycle){
        ...
        for (Node& node: m_Graph){
          for (Edge& edge: node.m_Edges){
            ...
            if (edgeRelaxed){
              change = true;
              m_UnexploredIndexes.erase(edge.GetDestinationNodePtr()->m_Index);
            }
          }
        }
...
int main() {
  ...
  OptimalExchange SPF(std::move(graph));
  std::cout << SPF.FindExchange() << std::endl;
  return 0;
}
